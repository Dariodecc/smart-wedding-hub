...
const Dashboard = () => {
  const { wedding, isLoading: weddingLoading } = useCurrentMatrimonio();
  ...
  const { data: dashboardData, isLoading: dataLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { count: famiglieCount, error: famiglieError } = await supabase
        .from('famiglie')
      ...
      // Fetch gruppi count
      const { count: gruppiCount, error: gruppiError } = await supabase
        .from('gruppi')
        .select('id', { count: 'exact', head: true })
        .eq('wedding_id', wedding.id);
  ...
  const stats = useMemo(() => {
    ...
    const { invitati, famiglieCount, gruppiCount, tavoli } = dashboardData;
...
const RSVPDonutChart = ({ data }: { data: { confermati: number; inAttesa: number; declinati: number } | undefined }) => {
  if (!data) return null;
  
  const chartData = [
    { name: 'Confermati', value: data.confermati, color: '#10B981' },
    { name: 'In Attesa', value: data.inAttesa, color: '#F59E0B' },
    { name: 'Declinati', value: data.declinati, color: '#EF4444' }
  ];
...
const AgeDistributionChart = ({ data }: { data: { neonati: number; bambini: number; ragazzi: number; adulti: number } | undefined }) => {
  if (!data) return null;
  
  const chartData = [
    { name: 'Neonati', value: data.neonati, color: '#EC4899', icon: '👶' },
    { name: 'Bambini', value: data.bambini, color: '#F59E0B', icon: '🧒' },
    { name: 'Ragazzi', value: data.ragazzi, color: '#3B82F6', icon: '👦' },
    { name: 'Adulti', value: data.adulti, color: '#8B5CF6', icon: '👨' }
  ];
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6">
            {chartData.map((item) => (
              <div
                key={item.name}
                className="p-3 rounded-lg border-2 transition-colors hover:border-gray-300"
...
const TablesProgressChart = ({ data, navigate }: { data: any[] | undefined; navigate: (path: string) => void }) => {
  if (!data) return null;
              ...
              >
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div className={cn(
                      ...
                      !isFull && !isAlmostFull && "bg-green-100"
                    )}>
                      <Table2 className={cn(
                        "h-5 w-5",
                        isFull && "text-red-600",
                  ...
                  <Badge className={cn(
                    ...
                    !isFull && !isAlmostFull && "bg-green-100 text-green-800"
                  )}>
                    {tavolo.percentuale}%
                  </Badge>
                </div>
                ...
                <div className="relative w-full h-3 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    className={cn(
                      ...
                      !isFull && !isAlmostFull && "bg-green-500"
                    )}
                    style={{ width: `${Math.min(tavolo.percentuale, 100)}%` }}
                  />
                </div>
                ...
                <div className="flex items-center justify-between mt-2">
                  <p className={cn(
                    ...
                    {isFull ? 'Tavolo completo' : 
                     isAlmostFull ? 'Quasi completo' : 
                     `${tavolo.capienza - tavolo.occupati} posti disponibili`}
                  </p>
